...
public class MouseCamera : MonoBehaviour
{
    ...
    private Vector2 velocityPlayer; // The current rotation velocity, in degrees
    ...
    private void Update() {
        ...
        velocityPlayer = new Vector2(
            0,
            Mathf.MoveTowards(velocityPlayer.y, wantedVelocity.y, acceleration.y * Time.deltaTime));
            
            velocityCam = new Vector2(
            Mathf.MoveTowards(velocityCam.x, wantedVelocity.x, acceleration.x * Time.deltaTime),
            0);
        rotation = new Vector2(velocityPlayer.y * Time.deltaTime, velocityCam.x * Time.deltaTime);
        rotation.y = ClampVerticalAngle(rotation.y);

        // Convert the rotation to euler angles
        transform.localEulerAngles = new Vector3(rotation.y, transform.localEulerAngles.x, 0);
        Camera.localEulerAngles = new Vector3(Camera.localEulerAngles.y, rotation.x, 0);
    }
}
